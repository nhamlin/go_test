...
func SomeNewFunction(xs []float64) float64 {
...
func average1(xs []float64) (avg float64) {
	total := 0.0
	for _, v := range xs {
		total += v
	}
...
// The following function is a more common interface
func average2(xs []float64) float64 {
	total := 0.0
	for _, v := range xs {
		total += v
	}
